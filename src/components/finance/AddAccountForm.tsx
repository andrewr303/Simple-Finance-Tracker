    ...
    <form onSubmit={handleSubmit} className="rounded-xl border border-border bg-card p-5 shadow-sm">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label htmlFor="current-balance" className="mb-1 block text-sm font-medium text-card-foreground">Current Balance ($)</label>
            <input id="current-balance" type="number" min={0} value={currentBalance} onChange={(e) => setCurrentBalance(e.target.value)} placeholder="0"
              className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
